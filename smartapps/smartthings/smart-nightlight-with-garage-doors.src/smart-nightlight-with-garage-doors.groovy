 ...
 */
definition(
    name: "Smart Nightlight with Garage Doors",
    namespace: "smartthings",
    author: "SmartThings",
    description: "Turns on lights when it's dark and motion is detected. Turns lights off when it light, some time after motion, or garage doors are closed. Also will keep light on or off when switch is pushed.",
...
def contactHandler(evt) {
	...
	if (!state.lightstayoff && !state.lightstayon) {
    	if (evt.value == "open") {
    		if (!enabled()) {
				log.debug "turning off lights due to door open"
		...
		if ((contact1.currentContact == "closed") && (contact2.currentContact == "closed")) {
			if (state.motionevt == "active") {
        		log.debug "turning on lights due to door close"
        		SwitchTurnedOnLight()
        	}    
    		if (state.motionevt == "inactive") {
        		log.debug "turning off lights due to door close"
        		SwitchTurnedOffLight()
        	}
		}
	}        
