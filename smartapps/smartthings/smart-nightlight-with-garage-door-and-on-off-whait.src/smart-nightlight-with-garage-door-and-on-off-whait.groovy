 ...
 */
definition(
    name: "Smart Nightlight with Garage Door and On / Off whait",
...
def motionHandler(evt) {
	...
	if (evt.value == "active") {
        ...
        if ((enabled() || ((contact1.currentContact == "closed") && (contact2.currentContact == "closed"))) && !state.lightstayoff && !state.lightstayon) {
			...
			lights.on()
			state.lastStatus = "on"
			runIn(2, SetLigthsStayOnStateAfterDelayMotion)
		}
...
def turnOffMotionAfterDelay() {
	...
	if (state.motionStopTime && state.lastStatus != "off") {
        ...
        if ((elapsed >= ((delayMinutes ?: 0) * 60000L) - 2000) && !state.lightstayoff && !state.lightstayon) {
            ...
            runIn(2, SetLigthsStayOffStateAfterDelayMotion)
		}
	}
}
...
def switchHandler(evt) {
    ...
    if(evt.value == "on")  {
		...
		state.lightstayoff = false	
		runIn(ondelayminutes*60, SetLigthsStayOnStateAfterDelaylightOff)		
    }
	if (evt.value == "off") {
    	unsubscribe (SetLigthsStayOnStateAfterDelaylightOff)
        state.lightstayoff = true
		state.lightstayon = false
        log.debug "Montion status after light off = ${state.lightstayoff}"
		runIn(offdelayminutes*60, SetLigthsStayOffStateAfterDelay)
    }
}

def SetLigthsStayOnStateAfterDelaylightOff() {
	log.trace "Setting turn on state after delay and turn light switch"
	state.lightstayon = false
    lights.off()
	state.lastStatus = "off"
    runIn(1, SetLigthsStayOffStateAfterDelay)
}
