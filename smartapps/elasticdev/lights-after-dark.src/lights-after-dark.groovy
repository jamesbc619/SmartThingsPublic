...
preferences {
    ...
    section("Turn on these lights") {
        input "light1", "capability.switch", title: "Light 1?"
        input "light2", "capability.switch", title: "Light 2?"
		...
		if("present" == evt.value) {
            ...
            if (light1.currentSwitch == "off")  {
                ...
                if (lightsDuration) {
                    ...
                    runIn(delayInSeconds, lightsOffHandler1)
                }
            }
            if (light2.currentSwitch == "off")  {
                light2.on()
                //If we turn off the lights after some period, set timer
                if (lightsDuration) {
                    def delayInSeconds = lightsDuration * 60
                    //No need to guard against multiple arrivals, 
                    //since calling 'runIn' again cancels any existing scheduled event
                    runIn(delayInSeconds, lightsOffHandler2)
                }
            }
        }
    }
 ...
 *	evt		The runIn event
 */
def lightsOffHandler1(evt)
{
    log.debug "runIn evt: ${evt}"
    light1.off()
}
def lightsOffHandler2(evt)
{
    log.debug "runIn evt: ${evt}"
    light2.off()
}
